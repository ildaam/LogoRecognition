...
def class_text_to_int(row_label, classes):
    if row_label not in classes:
        print('ROW LABEL',row_label)
        classes.append(row_label)
...
def create_tf_example(group, path, file_, classes_):
    #print ("GROUP", group)
    #print("PATH", path)
    ...
    for index, row in group.object.iterrows():
        ...
        classes.append(class_text_to_int(row['class'], classes_))
...
def main(_):
    ...
    classes_, classes_folders= class_list()
    ...
    for class_ in classes_folders:
        ...
        for i in range (0, len(group_list)):
            ...
            for k in range (0, len(ID_list)):
                ...
                if group.name == ID:
                    tf_example = create_tf_example(group, path, file_, classes_)
                    if tf_example != None:
